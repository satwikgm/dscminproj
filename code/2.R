...
df <- read.csv("data/dataset/child-mortality.csv")
...
ggplot(df2, aes(x = Year, y = Child.mortality.rate...., color = Entity)) +
  geom_line() +
...
df3 <- read.csv("data/dataset/imrace.csv")
names(df3)
summary(df3)

##########################################################################

data1 <- read.csv("data/dataset/filtered_data.csv")
data2 <- read.csv("data/dataset/imrace.csv")
dataCommon <- read.csv("data/dataset/global-child-mortality-timeseries.csv")
